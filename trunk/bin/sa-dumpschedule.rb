...
class Optparser
  ...
  def self.parse(args)
    ...
    opts = OptionParser.new do |opts|
      ...
      # Mandatory argument.
      opts.on("-s", "--store PATH", "path to schedule store") do |store|
        options.store=store
      end
      # Boolean switch.
...
puts "# Using SA schedule file #{storeFullPath}"

schedule = Schedule.instanceFromFile(storeFullPath)
puts "# found solution:"
puts schedule.to_s

puts " - Makespan: #{schedule.makespan()}"
puts " - Abs. Queuetime: #{schedule.absQueueTime()}"
puts " - Avg. Queuetime: #{schedule.avgQueueTime()}"
